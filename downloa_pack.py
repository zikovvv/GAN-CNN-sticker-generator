...
class StickerDownloader:
    def __init__(self, token, session=None, multithreading=4):
        ...
        self.token = token
        self.cwd = assure_folder_exists('packs', root=os.getcwd())
        if session is None:
    ...
    def download_sticker_set(self, sticker_set):
        ...
        download_path = swd#assure_folder_exists('webp', root=swd)
        downloads = []
...
if __name__ == "__main__":
    ...
    names = ["AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAADEATHAAAAAAAAAAAAAAAAAAA"]
